/**
 * The following code is a modification of Roelof Berg's code, the License note has been
 * preserved in its entirety, but the code was altered to use classes so that it could
 * be used neatly as a header-only library..
 * At the time, while the author recommends using codecvt_utf8 from the standard library,
 * the C++ committee no longer supports it, and deprecated it with no viable alternative.
 * Since there's no future for these standard functions, and alledgely they have security
 * vulnerabilities, I decided to use this custom implementation, as it is simpler than
 * the standard library one, since it doesn't rely on templates.
 *
 * Felipe Lopes, March 24th, 2026
 */
...
class Utf8Ucs2Converter {
public:
    Utf8Ucs2Converter() = delete;
