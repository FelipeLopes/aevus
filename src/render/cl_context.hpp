...
public:
    ...
    CLBuffer<T> createReadOnlyBuffer(const CLQueue& queue, size_t size) {
        return CLBuffer<T>(context, queue.commandQueue, CL_MEM_READ_ONLY, size);
    }
    template <typename T>
    CLBuffer<T> createWriteOnlyBuffer(const CLQueue& queue, size_t size) {
        return CLBuffer<T>(context, queue.commandQueue, CL_MEM_WRITE_ONLY, size);
    }
    template <typename T>
    CLBuffer<T> createReadWriteBuffer(const CLQueue& queue, size_t size) {
        return CLBuffer<T>(context, queue.commandQueue, CL_MEM_READ_WRITE, size);
    }
